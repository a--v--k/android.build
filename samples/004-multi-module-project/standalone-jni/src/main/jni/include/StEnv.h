...
public:
    ...
    {
        buf = buffer ? jenv->GetByteArrayElements(buffer, nullptr) : nullptr;
    }
...
public:
    JIntArray(JNIEnv *jenv, jintArray buffer)
        : jenv(jenv), buffer(buffer)
    {
        buf = buffer ? jenv->GetIntArrayElements(buffer, 0) : nullptr;
    }
...
public:
    JFloatArray(JNIEnv *jenv, jfloatArray buffer)
            : jenv(jenv), buffer(buffer)
    {
        buf = buffer ? jenv->GetFloatArrayElements(buffer, 0) : nullptr;
    }

    JFloatArray(JFloatArray&) = delete;
    JFloatArray(JFloatArray&&) = default;
    JFloatArray& operator=(JFloatArray const&) = delete;

    ~JFloatArray()
    {
        if (buffer && buf)
        {
            jenv->ReleaseFloatArrayElements(buffer, buf, 0);
        }
    }
...
public:
    JStringArray(JNIEnv *jenv, jobjectArray buffer)
            : jenv(jenv), buffer(buffer)
    {
        length = buffer ? jenv->GetArrayLength(buffer) : 0;
        buf = static_cast<jstring *>(calloc(length, sizeof(jstring)));
        for(int i=0; i < length; i++) {
            buf[i] = static_cast<jstring>(jenv->GetObjectArrayElement(buffer, i));
        }
    }

    JStringArray(JStringArray&) = delete;
    JStringArray(JStringArray&&) = default;
    JStringArray& operator=(JStringArray const&) = delete;
...
public:
        ...
        : jenv(jenv), buffer(buffer)
    {
        ...
        buf = buffer ? (T*)jenv->GetDirectBufferAddress(buffer) : nullptr;
    }
...
public:
    ...
    {
        buf = javaString ? jenv->GetStringUTFChars(javaString, nullptr) : nullptr;
    }

    JString(JNIEnv *jenv, jstring javaString, const char* defValue)
        : jenv(jenv), javaString(javaString)
    {
        buf = javaString ? jenv->GetStringUTFChars(javaString, nullptr) : defValue;
    }
