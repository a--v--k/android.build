...
val KNOWN_LANGS = setOf("it", "ru", "tt", "pl", "de", "es", "sk", "ko")

const val DEF_LOCALIZATION = "values"

fun checkLocalization(android: BaseExtension, appName: String? = null) {
    doTask(android, appName) { appLocalization -> checkAppLocalization(appLocalization) }
}

fun updateLocalization(android: BaseExtension, appName: String? = null) {
    doTask(android, appName) { appLocalization -> updateAppLocalization(appLocalization) }
}

fun doTask(android: BaseExtension, appName: String? = null, task: (AppLocalization) -> Unit) {
    android.sourceSets.forEach { sourceSet ->
        ...
        if (name == "implementation" || appName != null && name.startsWith(appName)) {
            sourceSet.res.sourceDirectoryTrees.forEach { tree ->
                if (tree.dir.exists()) {
                    val appLocalization = loadAppLocalization(tree.dir.toPath())
...
fun loadAppLocalization(path: Path): AppLocalization {
    val result = AppLocalization(absolutePath = path)

    path.listDirectories { p -> isLocalizationDir(p) }.forEach { absValuesPath ->
        val dir = loadLocalizationDir(absValuesPath)
...
fun loadLocalizationDir(absValuesPath: Path): LocalizationDir {
    val result = LocalizationDir(relativeDirPath = absValuesPath.getLastSegment(), lang = getLang(absValuesPath))
...
fun isLocalizationDir(path: Path): Boolean {
    ...
    if (name.startsWith("values-")) {
        ...
        if (KNOWN_LANGS.contains(locale)) {
            return true
        }
    }
    return false
